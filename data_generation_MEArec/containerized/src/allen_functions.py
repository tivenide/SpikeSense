...
# Patch for numpy scalar serialization compatibility (numpy 2.0+).
# MEArec's recursively_save_dict_contents_to_group checks isinstance(item, float),
# but np.float64 is no longer a subclass of float in numpy 2.0, causing a ValueError.
# This patch converts numpy scalars to Python native types before each save call.
try:
    import numpy as _np
    import MEArec.tools as _mrt
 
    _original_save = _mrt.recursively_save_dict_contents_to_group
 
    def _patched_save(h5file, path, dic):
        cleaned = {}
        for k, v in dic.items():
            if isinstance(v, dict):
                cleaned[k] = v  # _original_save recurses into dicts via _patched_save
            elif isinstance(v, _np.generic):
                cleaned[k] = v.item()
            else:
                cleaned[k] = v
        _original_save(h5file, path, cleaned)
 
    _mrt.recursively_save_dict_contents_to_group = _patched_save
...
def generating_template_of_allen_models(models_path, export_filename, probe_name, cell_types, seed=0):
    ...
    cell_models = [p for p in Path(models_path).iterdir() if p.is_dir()]
    print('cell_models:')
    print(cell_models)
    ...
    if len(cell_models) == 0:
        raise FileNotFoundError(
            f"No cell model directories found in '{models_path}'. "
            f"Expected sub-folders named like 'neuronal_model_<id>' "
            f"(see the readme-*.txt files in 'allen/models'). "
            f"Download the Allen cell models and place them in '{models_path}' "
            f"before generating the template library."
        )
